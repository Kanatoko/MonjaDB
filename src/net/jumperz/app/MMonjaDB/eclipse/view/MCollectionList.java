...
import net.jumperz.app.MMonjaDBCore.action.mj.MShowAllCollectionStatsAction;
import net.jumperz.app.MMonjaDBCore.action.mj.MShowAllCollectionStatsAction;
...
image = MUtil.getImage( parent.getShell().getDisplay(), "table_multiple.png" );
...
if( db != null )
	...
	String collNameWithNs = items[ 0 ].getText( 0 ); //waf.service
	if( collNameWithNs.indexOf( dbName ) == 0 )
		{
		if( isActive() )
			{
			actionManager.executeAction( "db." + collNameWithNs.substring( dbName.length() + 1 ) + ".find()" );
			}	
		}
	}
}
...
try
	...
	MSwtUtil.getTableColumnWidthFromProperties2( COLLLIST_TABLE, table, prop );
...
if( eventName.indexOf( event_showcollections + "_end" ) == 0 )
	{
